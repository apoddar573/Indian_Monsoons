...
f = open("parameterGAU.txt", "w")

for i in range(2008, 2015):
	name = "dailyGAU" + str(i) + ".csv"
	...
	plt.savefig("DailyGuwahatiRainfall" + str(i) + ".png")
	plt.close()

	model = ARIMA(dailyrain, order = (5, 0, 1))
	...
	plt.legend(loc = "best")
	plt.savefig("DailyGAU" + str(i) + str((5, 0, 1)) + ".png")
	plt.close()

...
f = open("parameterMNG.txt", "w")

for i in range(2008, 2015):
	name = "dailyMNG" + str(i) + ".csv"
	...
	plt.savefig("DailyMangaloreRainfall" + str(i) + ".png")
	plt.close()

	model = ARIMA(dailyrain, order = (2, 0, 0))
	...
	plt.title("RS: %.4f" %(sqrt(np.mean(sum((dailyrain - results)**2)))))
	plt.legend(loc = "best")
	plt.savefig("DailyMNG" + str(i) + str((2, 0, 0)) + ".png")
	plt.close()
	f.write("Parameters for the year " + str(i) + "\n")
